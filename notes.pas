...
uses
  ...
  FireDAC.Comp.Client, FireDAC.Comp.DataSet, vcl.Dialogs, RegularExpressionsCore, DataConnUnit;
...
begin
  ...
  regEx.Subject := ATable;
  try
    // Validate the table name to ensure it contains only valid characters (e.g., letters and numbers)
...
begin
  SetDataUpMapping;
//  fMapper.Query.SQL.Text := 'CREATE TABLE IF NOT EXISTS NOTES ( ID INTEGER PRIMARY KEY AUTOINCREMENT, NOTE TEXT )';
  fMapper.Query.ExecSQL;
end;

procedure TDataModule.saveNotesToDatabase(NoteText: string);
begin
//  fMapper.Query.SQL.Text := 'INSERT INTO NOTES (NOTE) VALUES (:NoteText)';
